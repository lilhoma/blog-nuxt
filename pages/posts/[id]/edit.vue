<template>
    <div class="container mx-auto w-1/2 py-8">
      ...
      <div class="mt-4">
        <button @click="deletePost" class="inline-block bg-red-600 hover:bg-red-700 text-white rounded px-4 py-2">Delete Post</button>
    </div>
    </div>
  </template>
  ...
  onMounted(async() => {
    try {
    post.value = await useNuxtApp().$apiFetch(`/api/postsAuth/${route.params.id}`)
